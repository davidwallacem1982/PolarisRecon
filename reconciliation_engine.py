...
def run_reconciliation():
    ...
    if not os.path.exists("data/external_file.csv") or not os.path.exists(
        "data/internal_base.csv"
    ):
    ...
    reconciled = internal_df.join(
        external_df, on="order_id", how="outer", suffix="_ext"
    )
    ...
    print(f"[*] Exportando relatórios detalhados para {os.path.abspath('reports')}...")
    ...
    print(
        f"    🎯  SLA STATUS:    {((total_time / 1200) * 100):.4f}% do limite (20 min)"
    )
    print("    " + "─" * 40 + "\n")
